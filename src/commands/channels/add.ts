...
export async function channelsAddCommand(
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `插件 ${catalogEntry.meta.label} 安装后无法加载。`
      : `未知频道：${String(opts.channel ?? "")}`;
    runtime.error(hint);
    runtime.exit(1);
    return;
  }
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(`频道 ${channel} 不支持添加操作。`);
    runtime.exit(1);
    return;
  }
  const accountId =
