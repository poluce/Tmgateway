...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      "DM 安全：默认为配对；未知 DM 会获得配对码。",
      `批准命令：${formatCliCommand("openclaw pairing approve <channel> <code>")}`,
      '公开 DM 需要 dmPolicy="open" + allowFrom=["*"]。',
      '多用户 DM：设置 session.dmScope="per-channel-peer"（或多账户频道使用 "per-account-channel-peer"）以隔离会话。',
...
export async function setupChannels(
  ...
  const configureChannel = async (channel: ChannelChoice) => {
    const adapter = getChannelOnboardingAdapter(channel);
    if (!adapter) {
      await prompter.note(`${channel} 尚不支持引导配置。`, "频道设置");
      return;
    }
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "暂时跳过",
          hint: `您可以稍后通过 \`${formatCliCommand("openclaw channels add")}\` 添加频道`,
        },
      ],
